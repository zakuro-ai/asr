...
parser.add_argument(
    "--num-workers", default=32, type=int, help="Number of workers used in data-loading"
)
parser.add_argument(
    "--window-size", default=0.02, type=float, help="Window size for spectrogram in seconds"
)
parser.add_argument(
    "--window-stride", default=0.01, type=float, help="Window stride for spectrogram in seconds"
)
parser.add_argument("--window", default="hamming", help="Window type for spectrogram generation")
# parser.add_argument('--hidden-size', default=800, type=int, help='Hidden size of RNNs')
# parser.add_argument('--hidden-layers', default=5, type=int, help='Number of RNN layers')
parser.add_argument("--hidden-size", default=480, type=int, help="Hidden size of RNNs")
...
parser.add_argument(
    "--max-norm", default=400, type=int, help="Norm cutoff to prevent explosion of gradients"
)
...
parser.add_argument(
    "--silent", dest="silent", action="store_true", help="Turn off progress tracking per iteration"
)
parser.add_argument(
...
parser.add_argument(
    "--log-dir", default="visualize/deepspeech_final", help="Location of tensorboard log"
)
parser.add_argument(
    "--log-params",
    dest="log_params",
...
parser.add_argument(
    "--noise-max", default=0.5, help="Maximum noise levels to sample from. Maximum 1.0", type=float
)
parser.add_argument(
    "--no-shuffle",
    dest="no_shuffle",
    ...
    if "output_file" not in kwargs:
        if "save_folder" in kwargs:
            if os.path.exists(kwargs["save_folder"]):
                kwargs["output_file"] = (
                    f"{kwargs['save_folder']}/deepspeech_jp_500_{kwargs['manifest']}.txt"
                )
    if "overwrite" in kwargs:
        kwargs["continue_from"] = None
    else:
        if "continue_from" in kwargs:
            kwargs["continue_from"] = (
                kwargs["continue_from"]
                if os.path.exists(kwargs["continue_from"])
                else kwargs["model_path"]
            )
    if "train_manifest" not in kwargs:
        kwargs["train_manifest"] = f"{kwargs['root_manifest']}/{kwargs['manifest']}-train.json"
    if "val_manifest" not in kwargs:
